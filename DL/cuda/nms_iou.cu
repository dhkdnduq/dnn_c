...
float Line::call(Vector const v) const { return (a * v.p.x + b * v.p.y + c); }
...
__device__ float IntersectionArea(float2 *mrect, float2 *mrect_shift,
                                  float2 *intersection) {
  int count = kCorners;
  for (int i = 0; i < kCorners; i++) {
    float2 intersection_shift[kPoints]{};
    for (int k = 0; k < count; k++) intersection_shift[k] = intersection[k];
...
#pragma unroll
    for (int k = 0; k < kPoints; k++) line_values_shift[k] = line_values[k];
    ...
    for (int k = 0; k < count; k++) intersection[k] = new_intersection[k];
  }
  ...
  if (count > 2) {
    for (int k = 0; k < count; k++)
      intersection_area += intersection[k].x * intersection_shift[k].y -
                           intersection[k].y * intersection_shift[k].x;
  }
...
__global__ void iou_cuda_kernel(int const numBoxes, int const numAnchors,
                                ...
                                float2 const *a_box_vals, float *iou_vals) {
  int t = blockIdx.x * blockDim.x + threadIdx.x;
  int stride = blockDim.x * gridDim.x;
  int combos = numBoxes * numAnchors;
  for (int tid = t; tid < combos; tid += stride) {
    float2 intersection[kPoints]{-1.0f, -1.0f, -1.0f, -1.0f, -1.0f, -1.0f,
...
#pragma unroll
    for (int b = 0; b < kCorners; b++) {
      if (b_box_vals[(static_cast<int>(tid / numAnchors) * kCorners + b)].x ==
          a_box_vals[(tid * kCorners + b) % (numAnchors * kCorners)].x)
        pad.x = 0.001f;
      else
        pad.x = 0.0f;
      if (b_box_vals[(static_cast<int>(tid / numAnchors) * kCorners + b)].y ==
          a_box_vals[(tid * kCorners + b) % (numAnchors * kCorners)].y)
        pad.y = 0.001f;
      else
        pad.y = 0.0f;
      intersection[b] = padfloat2(
          b_box_vals[(static_cast<int>(tid / numAnchors) * kCorners + b)], pad);
      rect1[b] =
          b_box_vals[(static_cast<int>(tid / numAnchors) * kCorners + b)];
      rect1_shift[b] =
          b_box_vals[(static_cast<int>(tid / numAnchors) * kCorners + b)];
      rect2[b] = a_box_vals[(tid * kCorners + b) % (numAnchors * kCorners)];
      rect2_shift[b] =
          a_box_vals[(tid * kCorners + b) % (numAnchors * kCorners)];
    }
    rotateLeft(rect1_shift, 4);
    rotateLeft(rect2_shift, 4);
    float intersection_area =
        IntersectionArea(rect2, rect2_shift, intersection);
    // Union
...
#pragma unroll
    for (int k = 0; k < kCorners; k++) {
      rect1_area +=
          rect1[k].x * rect1_shift[k].y - rect1[k].y * rect1_shift[k].x;
      rect2_area +=
          rect2[k].x * rect2_shift[k].y - rect2[k].y * rect2_shift[k].x;
    }
...
int iou(const void *const *inputs, void *const *outputs, int num_boxes,
  ...
  iou_cuda_kernel<<<blocksPerGrid, threadsPerBlock, 0, stream>>>(
      num_anchors, num_boxes, anchors, boxes, iou_vals);
  return 0;
}
