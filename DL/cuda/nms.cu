...
int nms(int batchSize, const void *const *inputs, void *const *outputs,
  ...
  if (!workspace || !workspace_size) {
    workspace_size = get_size_aligned<bool>(count);
    workspace_size += get_size_aligned<int>(count);
    workspace_size += get_size_aligned<int>(count);
    workspace_size += get_size_aligned<float>(count);
    workspace_size += get_size_aligned<float>(count);

    size_t temp_size_flag = 0;
    cub::DeviceSelect::Flagged(
        (void *)nullptr, temp_size_flag, cub::CountingInputIterator<int>(count),
        (bool *)nullptr, (int *)nullptr, (int *)nullptr, count);
    size_t temp_size_sort = 0;
    cub::DeviceRadixSort::SortPairsDescending(
        (void *)nullptr, temp_size_sort, (float *)nullptr, (float *)nullptr,
  ...
  for (int batch = 0; batch < batchSize; batch++) {
    ...
    cub::DeviceSelect::Flagged(workspace, workspace_size,
                               cub::CountingInputIterator<int>(0), flags,
    ...
    cub::DeviceRadixSort::SortPairsDescending(workspace , workspace_size , scores , scores_sorted , indices , indices_sorted , 
