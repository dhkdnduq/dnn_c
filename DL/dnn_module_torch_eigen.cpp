...
void dnn_module_torch::load_anomaly_detection() {
  ...
  anomaly_rand_index = torch::tensor(
      {1632, 39,   990,  1740, 797,  661,  1698, 1623, 1782, 1064, 205,  911,
       ...
       539,  1670, 527,  958,  13,   1145, 166,  1604, 722,  1518, 1378, 514,  744,  211,  1475, 1665, 1290, 879,  1217, 1319, 153,  1284})
          .to(at::kCUDA);
...
bool dnn_module_torch::predict_anomaly_detection(unsigned char* buf, int buf_w, int buf_h, int buf_channel) 
{
  ...
  try {
    ...
    embedding_vectors = embedding_concat(embedding_vectors, output3);
    ...
    float* anomaly_ptr = anomaly_mean.data_ptr<float>();
    Eigen::Map<MatrixXf_rm> e_anomaly_mean(anomaly_ptr, anomaly_mean.size(0), anomaly_mean.size(1));
  
    // auto anomaly_mean_mat = tensor2dToMat(anomaly_mean);
    ...
    auto embedding_vertors_begin_node = embedding_vectors[0];
    float* embedding_ptr = embedding_vertors_begin_node.data_ptr<float>();
    Eigen::Map<MatrixXf_rm> e_embedding(  embedding_ptr, embedding_vertors_begin_node.size(0), embedding_vertors_begin_node.size(1));
...
#pragma omp parallel for
    for (int i = 0; i < H * W; i++) {
      ...
      auto conv_inv = anomaly_conv_inv.index({Slice(None, None), Slice(None, None), i});
      float* anomaly_convi_ptr = conv_inv.data_ptr<float>();
      Eigen::Map<MatrixXf_rm> e_anomaly_convi(  anomaly_convi_ptr, conv_inv.size(0), conv_inv.size(1));
      auto delta = sample - mean;
      auto dot = (delta.transpose() * e_anomaly_convi);
      auto m = dot * delta;
      dist_list[i] = std::sqrt(m.value());
